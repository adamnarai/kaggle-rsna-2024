...
class ForaminalROIModel(nn.Module):
    def __init__(self, base_model, num_classes, in_channels=None, pretrained=True):
        ...
        # self.classifier = nn.Linear(128 + 5 + 2, num_classes)

    def forward(self, x_sagt1, level, side):
        # x_sagt1 = self.model_sagt1(x_sagt1)
        # x = self.classifier(torch.cat((x_sagt1, level, side), dim=1))
...
class SubarticularROIModelV2(nn.Module):
    ...
    def forward(self, x_axi, level, side):
        return self.model_axi(x_axi)
