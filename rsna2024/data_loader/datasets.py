...
class RSNADataset(Dataset):
    ...
    def get_series_with_coord(self, study_id, series_description, img_num, series_id_selection='random'):
        x = np.zeros((*self.resolution, img_num), dtype=np.float32)
        ...
        if slice_num > img_num:
            ...
            file_list = [None] * ((img_num - slice_num) // 2) + file_list + [None] * ((img_num - slice_num) // 2 + (img_num - slice_num) % 2)
...
class RSNAMeanposDataset(RSNADataset):
    def __getitem__(self, idx):
        row = self.df.iloc[idx]
        label = self.df[self.out_vars].iloc[idx].values
        label = np.nan_to_num(label.astype(float), nan=0).astype(np.int64)
...
class RSNASplitCoordDataset(RSNADataset):
    ...
    def __getitem__(self, idx):
        ...
        x1, coord1, slicenum1 = self.get_series_with_coord(row.study_id, 'Sagittal T2/STIR', img_num=self.img_num[0])
        ...
        for i in range(5):
            if np.isnan(coord1[i, :]).any():
                ...
                heatmaps.append(gaussian_heatmap(*self.resolution, coord1[i,:], std_dev=self.heatmap_std))
